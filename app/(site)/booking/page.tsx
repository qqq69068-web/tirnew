...
const inputCls =
  "w-full border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-900 bg-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-teal-500 transition-colors";

const CATEGORIES = [
  { value: "truck", label: "🚛 Вантажівка" },
  { value: "trailer", label: "🚜 Причіп / Напівпричіп" },
  ...
  if (submitted) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center px-4">
        ...
        <button
          onClick={() => { setSubmitted(false); setForm({ name: "", phone: "", carMake: "", carModel: "", carYear: "", carCategory: "truck", service: "", description: "", date: "" }); }}
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input id="phone" name="phone" type="tel" required value={form.phone} onChange={handleChange}
              placeholder="+380 50 000 00 00" autoComplete="tel" inputMode="numeric" className={inputCls} />
          </div>
        </div>
      </fieldset>
      ...
      <fieldset>
        ...
        <div className="grid grid-cols-3 gap-3">
          {CATEGORIES.map((cat) => (
            <label
              key={cat.value}
              className={`flex flex-col items-center justify-center gap-1 cursor-pointer rounded-xl border-2 py-3 px-2 text-center text-sm font-medium transition-colors ${
                form.carCategory === cat.value
                  ? "border-teal-500 bg-teal-50 text-teal-700"
                  : "border-gray-200 bg-white text-gray-600 hover:border-teal-300"
              }`}
            >
              <input
                type="radio" name="carCategory" value={cat.value}
                checked={form.carCategory === cat.value}
                onChange={handleChange} className="sr-only"
              />
              <span className="text-xl">{cat.label.split(" ")[0]}</span>
              <span className="text-xs leading-tight">{cat.label.split(" ").slice(1).join(" ")}</span>
            </label>
          ))}
        </div>
      </fieldset>
      ...
      <fieldset>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            <label htmlFor="carMake" className="block text-sm font-medium text-gray-700 mb-1">Марка *</label>
            <input id="carMake" name="carMake" required value={form.carMake} onChange={handleChange}
              placeholder="Volvo" className={inputCls} />
          </div>
          <div>
            <label htmlFor="carModel" className="block text-sm font-medium text-gray-700 mb-1">Модель *</label>
            <input id="carModel" name="carModel" required value={form.carModel} onChange={handleChange}
              placeholder="FH 500" className={inputCls} />
          </div>
          <div>
            <label htmlFor="carYear" className="block text-sm font-medium text-gray-700 mb-1">Рік</label>
            <input id="carYear" name="carYear" type="number" min="1990" max="2026"
              value={form.carYear} onChange={handleChange} placeholder="2019" className={inputCls} />
          </div>
        </div>
      </fieldset>
      ...
      <fieldset>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            <label htmlFor="date" className="block text-sm font-medium text-gray-700 mb-1">Бажана дата</label>
            <input id="date" name="date" type="date" value={form.date} onChange={handleChange}
              min={new Date().toISOString().split("T")[0]} className={inputCls} />
          </div>
        </div>
        <div className="mt-4">
          <label htmlFor="description" className="block text-sm font-medium text-gray-700 mb-1">Опис проблеми</label>
  ...
  return (
    <main className="min-h-screen bg-[#f7f6f2]">
      <section className="bg-[#0f1923] text-white py-20 px-4 text-center">
        <p className="text-sm uppercase tracking-widest text-teal-400 mb-2 font-medium">Онлайн-запис</p>
        <h1 className="text-4xl md:text-5xl font-bold mb-3">Запишіться на сервіс</h1>
        <p className="text-gray-400 max-w-lg mx-auto text-sm">
          Залиште заявку — ми зв&apos;яжемось для підтвердження зручного часу.
        </p>
      </section>
