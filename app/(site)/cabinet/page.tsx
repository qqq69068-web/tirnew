...
function AuthForm({ onLogin }: { onLogin: (client: Client) => void }) {
  ...
  const sendCode = async () => {
    ...
    const res = await fetch("/api/client/send-magic-link", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ email }),
    });
    setLoading(false);
    if (res.ok) { setSent(true); setResendCd(60); }
  ...
  if (sent) return (
    <div className="cb-auth-wrap">
      ...
      <form onSubmit={submitCode} className="cb-auth-form">
        <div className="cb-field">
          <label className="cb-label">Введіть код</label>
      ...
      <div className="cb-auth-resend">
        {resendCd > 0 ? (
          <span>Надіслати ще раз через {resendCd} с</span>
        ) : (
          <button className="cb-auth-back" onClick={() => { setCode(""); setError(""); sendCode(); }}>
            Надіслати код повторно
          </button>
        )}
      </div>
      <button className="cb-auth-back" style={{ marginTop: 8 }} onClick={() => { setSent(false); setCode(""); setError(""); }}>
        ← Ввести інший email
      </button>
    </div>
  );
    ...
    <div className="cb-auth-wrap">
      ...
      <form onSubmit={(e) => { e.preventDefault(); sendCode(); }} className="cb-auth-form">
        ...
        <button type="submit" disabled={loading} className={`cb-submit-btn ${loading ? "cb-submit-btn--loading" : ""}`}>
          {loading ? <span className="cb-spinner" /> : <><span>Отримати код</span><ArrowRight size={15} /></>}
        </button>
      </form>
  ...
  return (
    <>
      <style>{`
        ...
        .cb-eyebrow { font-size: 10px; font-weight: 700; letter-spacing: 0.16em; text-transform: uppercase; color: #4a9e6b; margin-bottom: 10px; }
        .cb-page-title { font-size: clamp(1.6rem, 4vw, 2.6rem); font-weight: 800; color: var(--text); line-height: 1.15; }
        ...
        .cb-user-header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 24px; }
        ...
        .cb-logout-btn { display: flex; align-items: center; gap: 6px; font-size: 13px; color: var(--text-muted); background: none; border: none; cursor: pointer; padding: 6px 10px; border-radius: 8px; transition: background 0.18s, color 0.18s; }
        ...
        .cb-stat-card { background: var(--surface); border: 1px solid var(--border); border-radius: 12px; padding: 16px 12px; text-align: center; transition: box-shadow 0.22s, transform 0.22s; }
        ...
        .cb-card { background: var(--surface); border: 1px solid var(--border); border-radius: 14px; padding: 18px 20px; transition: box-shadow 0.22s, transform 0.22s; }
        ...
        .cb-cost-pill { display: inline-flex; align-items: center; gap: 5px; font-size: 12px; color: var(--text-muted); background: var(--surface2); border: 1px solid var(--border); border-radius: 99px; padding: 3px 10px; }
        ...
        .cb-progress-step { display: inline-flex; align-items: center; gap: 5px; font-size: 11px; font-weight: 600; padding: 4px 10px; border-radius: 99px; border: 1px solid transparent; transition: background 0.2s, color 0.2s; }
        ...
        .cb-section-eyebrow { font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.12em; color: var(--text-faint); }
        ...
        .cb-empty-link { display: inline-flex; align-items: center; gap: 5px; font-size: 13px; font-weight: 600; color: #4a9e6b; text-decoration: none; border-bottom: 1px solid rgba(74,158,107,0.30); padding-bottom: 1px; transition: border-color 0.18s; }
        ...
        /* ── Auth ── */
        .cb-auth-wrap { max-width: 360px; margin: 0 auto; padding: 0 16px; display: flex; flex-direction: column; align-items: center; text-align: center; }
        .cb-auth-icon-wrap { width: 56px; height: 56px; border-radius: 14px; background: rgba(74,158,107,0.10); display: flex; align-items: center; justify-content: center; margin-bottom: 14px; }
        ...
        .cb-auth-form  { width: 100%; background: var(--surface); border: 1px solid var(--border); border-radius: 14px; padding: 24px 20px; display: flex; flex-direction: column; gap: 14px; margin-top: 22px; }
        .cb-auth-hint  { font-size: 11px; color: var(--text-faint); margin-top: 12px; }
        .cb-auth-error { font-size: 12px; color: #e05555; text-align: center; margin: -4px 0; }
        .cb-auth-resend { margin-top: 14px; font-size: 12px; color: var(--text-faint); }
        .cb-auth-back  { margin-top: 4px; font-size: 13px; color: #4a9e6b; background: none; border: none; cursor: pointer; text-decoration: underline; text-underline-offset: 3px; }
